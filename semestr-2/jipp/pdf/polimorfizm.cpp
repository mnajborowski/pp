...
public:
  ...
  // Je¿eli chcemy korzystaæ z polimorfizmu niezbêdnej est zdefiniowanie
...
public:
  // Metoda czysto-wirtualna (pure virtual) -- nie mo¿na utworzyæ instancji
  // klasy Weapon
...
// --------------------------------------------------
// Funkcja przyjmie jako argument dowolny obiekt, którego klas¹ bazow¹ jest
// Weapon
void attack_monster_ref(const Weapon &weapon) {
  ...
  std::cout << "Aiming!\n";
  std::cout << "Steady!\n";
  // dziêki polimorfizmowi, wywo³ywana jest funkcja attack z klasy zwi¹zanej z
...
void attack_with_everything_ptr(
  ...
  for (const auto &weapon : weapons) { // poszczególne elementy kolekcji s¹ typu
...
// Na liœcie argumentów mo¿na umieœciæ zarówno referencjê...
void print_item_ref(const Item &item) {
  std::cout << item.get_name() << ", " << item.get_price() << "gp\n";
}
// ...jak i wskaŸnik na klasê bazow¹
...
void shop_list(const std::vector<std::unique_ptr<T>> &items) {
  for (auto &item : items) {    // poszczególne elementy kolekcji s¹ typu
                                // std::unique_ptr<T>
    print_item_ref(*item);      // wy³uskujemy element
                                // i przekazujemy przez referencjê
    print_item_ptr(item.get()); // przekazujemy surowy wskaŸnik
  }
}
...
int main() {
  ...
  // polimorfizm z wykorzystaniem referencji
  {
    ...
    // Kolekcje swords i maces mog¹ przechowywaæ tylko obiekty typu Sword i
    ...
    // attack_monster_ref.
    // Jako argument przyjmuje ona referencje na Weapon. Mo¿na przekazaæ do niej
    // dowolny obiekt, który dziedziczy poœrednio lub bezpoœrednio po typie
    // Weapon.
    ...
    // Wywo³uje funkcjê szablonow¹, która jako argument przyjmuje kolekcjê o
    // dowolnym typie elementów. Zawiera kod analogiczny z linijek wy¿ej.
  ...
  // polimorfizm z wykorzystaniem wskaŸników
  {
    print_header("Polimorfizm z wykorzystaniem wskaznikow", '#');

    // Kolekcja weapons mo¿e przechowywaæ wskaŸniki na obiekty dowolnych typów
    // pod warunkiem, ¿e klas¹ nadrzêdn¹ jest Weapon
    ...
    // Próbuj¹c wywo³aæ poni¿sze dostaniemy b³ad, attack_with_everything_ptr
    // wymaga obiektów z klas¹ bazow¹ co najmniej Weapon
